...
func NewImageHandler(c *gin.Context) {
	...
	image.PublishedAt = time.Now()
	cap := capture.Capture{}
	cap.CaptureImage(image.ID)
...
func DeleteImageHandler(c *gin.Context) {
	...
	cap := capture.Capture{}
	cap.DeleteImage(images[index].ID)
	images = append(images[:index], images[index+1:]...)

	c.JSON(http.StatusOK, gin.H{"message": "Image has been deleted"})
}
...
func main() {
	router := gin.Default()
	router.POST("/api/images", NewImageHandler)
